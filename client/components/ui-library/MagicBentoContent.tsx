...
const BentoCardGrid: React.FC<{ children: React.ReactNode; gridRef?: React.RefObject<HTMLDivElement | null>; }> = ({ children, gridRef }) => (
    <div className="magic-bento-section grid gap-2 p-3 max-w-[54rem] select-none relative" style={{ fontSize: 'clamp(1rem, 0.9rem + 0.5vw, 1.5rem)' }} ref={gridRef}>
        {children}
    </div>
);
...
export const MagicBentoContent: React.FC<BentoProps> = ({
    ...
    glowColor = DEFAULT_GLOW_COLOR, clickEffect = true, enableMagnetism = true
}) => {
    ...
    .magic-bento-grid { grid-template-columns: 1fr; grid-auto-rows: 120px; width: 100%; max-width: 800px; margin: 0 auto; padding: 0.5rem; }
    @media (min-width: 480px) { .magic-bento-grid { grid-template-columns: repeat(2, 1fr); } }
    @media (min-width: 1024px) {
      .magic-bento-grid { grid-template-columns: repeat(4, 1fr); }
      .magic-bento-grid .magic-bento-card:nth-child(3) { grid-column: span 2; grid-row: span 2; }
      .magic-bento-grid .magic-bento-card:nth-child(4) { grid-column: 1 / span 2; grid-row: 2 / span 2; }
      .magic-bento-grid .magic-bento-card:nth-child(6) { grid-column: 4; grid-row: 3; }
    }
    .magic-bento-card--border-glow::after {
      ...
      border-radius: inherit; -webkit-mask: linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0); -webkit-mask-composite: xor; mask: linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0); mask-composite: exclude;
      pointer-events: none; opacity: 1; transition: opacity 0.3s ease; z-index: 1;
    }
    .magic-bento-card--border-glow:hover::after { opacity: 1; }
    .magic-bento-card--border-glow:hover { box-shadow: 0 4px 20px rgba(46, 24, 78, 0.4), 0 0 30px rgba(${glowColor}, 0.2); }
    .magic-bento-particle::before { content: ''; position: absolute; top: -2px; left: -2px; right: -2px; bottom: -2px; background: rgba(${glowColor}, 0.2); border-radius: 50%; z-index: -1; }
    .magic-bento-particle-container:hover { box-shadow: 0 4px 20px rgba(46, 24, 78, 0.2), 0 0 30px rgba(${glowColor}, 0.2); }
    .magic-bento-clamp-1 { display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 1; line-clamp: 1; overflow: hidden; text-overflow: ellipsis; }
    .magic-bento-clamp-2 { display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 2; line-clamp: 2; overflow: hidden; text-overflow: ellipsis; }
    @media (max-width: 479px) {
      .magic-bento-grid { grid-template-columns: 1fr; grid-auto-rows: 120px; width: 100%; margin: 0 auto; padding: 0.25rem; gap: 0.5rem; }
      .magic-bento-grid .magic-bento-card { width: 100%; height: 100%; min-height: 0; }
    }
  `, [glowColor]);

    return (
        <div className="flex items-center justify-center p-12 bg-[#201E1D] min-h-[600px] w-full relative">
            <style>{styles}</style>
            {enableSpotlight && (
                        ...
                        if (enableStars) {
                            return (
                                <ParticleCard key={index} className={baseClassName} style={cardStyle} disableAnimations={shouldDisableAnimations} particleCount={particleCount} glowColor={glowColor} enableTilt={enableTilt} clickEffect={clickEffect} enableMagnetism={enableMagnetism}>
                                    <div className="magic-bento-card__header flex justify-between gap-1.5 relative text-white">
                                        <span className="magic-bento-card__label text-xs opacity-70">{card.label}</span>
                                    </div>
                                    <div className="magic-bento-card__content flex flex-col relative text-white">
                                        <h3 className={`magic-bento-card__title font-medium text-sm m-0 mb-0.5 ${textAutoHide ? 'magic-bento-clamp-1' : ''}`}>{card.title}</h3>
                                        <p className={`magic-bento-card__description text-[10px] leading-3 opacity-80 ${textAutoHide ? 'magic-bento-clamp-2' : ''}`}>{card.description}</p>
                                    </div>
                                </ParticleCard>
                            );
                        }
